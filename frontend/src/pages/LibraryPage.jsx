...
export default function LibraryPage() {
  const { user } = useAuth();
  const [activeTab, setActiveTab] = useState('bookmarks'); // 'bookmarks' | 'watched' | 'suggestions'
  ...
  const activeItems =
    activeTab === 'bookmarks' ? bookmarks : watched;
  ...
  return (
    <div className="space-y-8 pb-16">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3">
            <div className="px-4 py-2.5 rounded-2xl bg-slate-900 border border-slate-800 text-center">
              <span className="block text-[11px] text-brand-subtle font-medium">
                Bookmarked
              </span>

              <span className="text-sm font-bold text-brand-accent">
                {bookmarks.length} Titles
              </span>
            </div>
      ...
      {/* Streaming Platform Summary Widget */}
        ...
        <div className="p-4 rounded-2xl bg-brand-card/80 border border-brand-border flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {platformSummary.slice(0, 4).map(([name, count]) => (
              <span
                key={name}
                className="px-2.5 py-1 rounded-lg bg-slate-900 border border-slate-800 text-[11px] font-medium text-brand-muted"
              >
                {name}:{' '}
